...
class compile_and_run {
  ...
  static void clean(const std::string& filename) {
    ...
    if (file_util::is_exist(src)) unlink(src.c_str());

    auto compile_error = path_util::compile_error(filename);
    if (file_util::is_exist(compile_error)) unlink(compile_error.c_str());

    auto exe = path_util::exe(filename);
    if (file_util::is_exist(exe)) unlink(exe.c_str());

    auto __stdin = path_util::_stdin(filename);
    if (file_util::is_exist(__stdin)) unlink(__stdin.c_str());

    auto __stdout = path_util::_stdout(filename);
    if (file_util::is_exist(__stdout)) unlink(__stdout.c_str());

    auto __stderr = path_util::_stderr(filename);
    if (file_util::is_exist(__stderr)) unlink(__stderr.c_str());
  }
 ...
 public:
  static std::string start(const std::string& in_json) {
    ...
    if (!file_util::write_file(path_util::src(filename), code)) {
    ...
    if (!compiler::start(filename)) {
      std::string temp;
      file_util::read_file(path_util::compile_error(filename), temp);
    ...
    if (ret < 0) {
      ...
      file_util::read_file(path_util::_stdout(filename), _stdout);
      file_util::read_file(path_util::_stderr(filename), _stderr);
